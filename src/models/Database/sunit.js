import {SUNIT_list,SUNIT_add,SUNIT_update,SUNIT_delete} from '../../services/index';
...
export default {
  ...
  effects: {
    *gettransportgoods(action,{call,put}){
      const dat = yield call(SUNIT_list,action);
      if(!dat.err){
    ...
    *addtransportgoods(action,{call,put}){
      const dat = yield call(SUNIT_add,action);
      if(!dat.err){
    ...
    *edittransportgoods(action,{call,put}){
      const dat = yield call(SUNIT_update,action);
      if(!dat.err){
        const {Success,Message} = dat.data;
        if(Success){
          let page=action.pageIndex;
    ...
    *deletetransportgoods(action,{call,put}){
      const dat = yield call(SUNIT_delete,action);
      if(!dat.err){
