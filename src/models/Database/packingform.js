import {PACK_list,PACK_add,PACK_update,PACK_delete} from '../../services/index';
import {message} from 'antd';
import {loadsh} from '../../utils/fun_config'

export default {
  namespace: 'packingform',
  state: {
    tData: [],
    item: {},
    loading: false,
  ...
  effects: {
    *gettransportgoods(action,{call,put}){
      const dat = yield call(PACK_list,action);
      if(!dat.err){
    ...
    *addtransportgoods(action,{call,put}){
      const dat = yield call(PACK_add,action);
      if(!dat.err){
    ...
    *edittransportgoods(action,{call,put}){
      const dat = yield call(PACK_update,action);
      if(!dat.err){
        const {Success,Message} = dat.data;
        if(Success){
          let page=action.pageIndex;
    ...
    *deletetransportgoods(action,{call,put}){
      const dat = yield call(PACK_delete,action);
      if(!dat.err){
