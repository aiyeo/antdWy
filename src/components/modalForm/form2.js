...
class ModForm extends React.Component {
  ...
  changeInput(e,setvalue){
    ...
    showCarlist(){
       //打开选择车辆弹框，获取数据
       this.props.dispatch({type:'loadingcar/showCarModal'});
      ...
      return
       ...
       //显示选择驾驶人弹框
   showDriverList(){
    this.props.dispatch({type:'loadingcar/showDriverModal'});
  ...
  render() {
    const {
      fields,showCancel,noBtn,otherFields,getAllLoading,loadingIndex
    } = this.props;
    ...
    return (
      <div className={styles.formWrapper}>
        <Form onSubmit={this.handleSubmit} ref={(c) => { this.form = c; this.props.cb && this.props.cb(this.handleSubmit) }}>
            ...
            otherFields=='loading'?
	            <div>
				        <Row style={{marginBottom:'20px'}}>
				          <Col span={16}>
				          <ValiInput
				          spans={24} required={true} max={7} min={7} patter={/^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1}$/} message='请输入正确的车牌号'
          ...
          {
            otherFields=='loadingEdit'?
	            <div>
				        <Row style={{marginBottom:'20px'}}>
				          <Col span={16}>
				          <ValiInput
				          spans={24} required={true} labelspan={9} inputspan={14} handleChange={(e)=>this.changeInput(e,'carNo')}
				          labelText='车牌号' inputValue={this.props.carNo}  />
				          </Col>
				          <Col span={2}>
				            <Button size='large' style={{marginLeft:'12px'}} onClick={()=>this.showCarlist()}>选择车辆</Button>
